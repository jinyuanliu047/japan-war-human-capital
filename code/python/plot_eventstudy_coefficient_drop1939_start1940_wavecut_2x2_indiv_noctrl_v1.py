#!/usr/bin/env python3
"""2x2 panel for individual-level coefficients without controls."""
...
def _load(wave: str) -> pd.DataFrame:
    p = TEMP / f"eventstudy_coefficient_drop1939_start1940_{wave}_indiv_noctrl_v1.csv"
    df = pd.read_csv(p)
...
def main() -> None:
    ...
    fig.suptitle(
        "Cohort Coefficients (Individual eduy no controls, Drop 1939, Treated >= 1940)",
        x=0.01,
        ha="left",
        fontsize=15,
        fontweight="bold",
    ...
    out = FIG / "eventstudy_coefficient_drop1939_start1940_wavecut_2x2_indiv_noctrl_v1.png"
