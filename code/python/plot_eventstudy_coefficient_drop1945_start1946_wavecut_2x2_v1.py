#!/usr/bin/env python3
"""Generate a 2x2 event-study panel for the 1946 cutoff appendix figure."""
...
def _load(wave: str) -> pd.DataFrame:
    p = TEMP / f"eventstudy_coef_drop1945_start1946_main_{wave}_v1.csv"
    df = pd.read_csv(p)
    df = df.rename(columns={"beta": "coefficient"})
...
def main() -> None:
    plt.rcParams.update(
        {
            ...
            "font.family": "serif",
            "font.serif": ["Libertinus Serif", "Times New Roman", "Songti SC", "Noto Serif CJK SC", "DejaVu Serif"],
            "font.size": 11,
    ...
    for i, w in enumerate(["1982", "1990", "2000"]):
        ax = axes[i]
        d = dfs[w]
        ax.axvspan(1946, hi, color="#ECE9E3", alpha=0.45, zorder=0)
        ...
        ax.axvline(1946, color="#B03A2E", linestyle=(0, (5, 3)), linewidth=1.0)
        ax.set_xlim(1931, hi)
    ...
    out = FIG / "eventstudy_coefficient_drop1945_start1946_wavecut_2x2_v1.png"
