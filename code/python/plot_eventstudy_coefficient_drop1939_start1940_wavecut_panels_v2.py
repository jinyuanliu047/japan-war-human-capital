#!/usr/bin/env python3
"""Generate 4 individual coefficient plots + a 2x2 panel (with markers)."""
...
def _plot_single_with_ci(df: pd.DataFrame, title: str, color: str, marker: str, out_prefix: Path) -> None:
    ...
    ax.plot(df["birthyr"], df["coefficient"], color=color, linewidth=2.0, marker=marker, markersize=3.8, zorder=2)
...
def main() -> None:
    ...
    _plot_single_with_ci(
        ...
        "#1F2D4F",
        "o",
        FIG / "eventstudy_coefficient_drop1939_start1940_wavecut_A_1982_v1",
    )
    _plot_single_with_ci(
        dfs["1990"],
        "B. Census 1990",
        "#7A1F2B",
        "^",
        FIG / "eventstudy_coefficient_drop1939_start1940_wavecut_B_1990_v1",
    )
    _plot_single_with_ci(
        dfs["2000"],
        "C. Census 2000",
        "#2E4A3F",
        "s",
        FIG / "eventstudy_coefficient_drop1939_start1940_wavecut_C_2000_v1",
    )
    _plot_single_overlay(
        dfs,
        FIG / "eventstudy_coefficient_drop1939_start1940_wavecut_D_overlay_v1",
    )
    _plot_2x2(
        dfs,
        FIG / "eventstudy_coefficient_drop1939_start1940_wavecut_2x2_v2",
    )

    print(FIG / "eventstudy_coefficient_drop1939_start1940_wavecut_2x2_v2.png")
