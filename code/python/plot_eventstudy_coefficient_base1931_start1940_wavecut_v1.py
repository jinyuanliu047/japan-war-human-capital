#!/usr/bin/env python3
"""Plot cohort-specific treatment coefficients with wave-specific cutoffs."""
...
def main() -> None:
    ...
    summary = pd.read_csv(TEMP / "eventstudy_coefficient_base1931_start1940_wavecut_summary_v1.csv")
    for c in ["b_did", "se_did", "p_did", "p_pre_equalbase", "p_post_equalbase", "sample_hi"]:
    ...
    for ax, (wave, color, title) in zip(axes, specs):
        df = pd.read_csv(TEMP / f"eventstudy_coefficient_base1931_start1940_{wave}_v1.csv")
        for c in ["birthyr", "coefficient", "lb", "ub", "startyr", "baseyr", "sample_hi"]:
            df[c] = pd.to_numeric(df[c], errors="coerce")
        df = df.sort_values("birthyr")

        row = summary[summary["wave"].astype(str) == wave].iloc[0]
        start = int(df["startyr"].dropna().iloc[0])  # 1940
        base = int(df["baseyr"].dropna().iloc[0])    # 1931
        ...
        txt = (
            ...
            f"Pre(equal-to-base) p={_fmt_p(row['p_pre_equalbase'])}\n"
            f"Post(equal-to-base) p={_fmt_p(row['p_post_equalbase'])}"
        )
        ax.text(
            0.02,
            0.98,
    ...
    fig.suptitle(
        "Cohort Coefficients (Base 1931, Treated >= 1940, Wave-Specific Cutoffs)",
        x=0.01,
        ha="left",
        fontsize=14.8,
        fontweight="bold",
    ...
    out = FIG / "eventstudy_coefficient_base1931_start1940_wavecut_v1.png"
