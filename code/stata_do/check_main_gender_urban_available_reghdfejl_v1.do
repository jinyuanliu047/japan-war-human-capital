*******************************************************
* Gender all available waves + urban-rural available waves
* Uses prebuilt subgroup panels in data/temp/heterogeneity_groups_v1
*******************************************************
...
global paneldir "${proj}/data/temp/heterogeneity_groups_v1"
...
program define _run_group, rclass
    ...
    capture confirm variable minority_share
    if _rc != 0 gen minority_share = .
...
foreach w in 1982 1990 2000 {
    foreach g in male female {
        capture confirm file "${paneldir}/panel_`w'_`g'.dta"
        if !_rc {
            local m = "g`w'_`g'"
            quietly _run_group, wave(`w') group("`g'") model("`m'")
            local gmodels "`gmodels' `m'"
            use `gsum', clear
            local n = _N + 1
            set obs `n'
            replace wave = "`w'" in `n'
...
esttab `gmodels' using "${proj}/result/table/main_individual_heterogeneity_gender_reghdfejl_v3.rtf", ///
    replace keep(1.post#c.ln_martyr_per100k_1953) ///
    scalar(Group Controls County_FE Cohort_FE) se r2 ar2 star(* 0.1 ** 0.05 *** 0.01)
esttab `gmodels' using "${proj}/result/table/main_individual_heterogeneity_gender_reghdfejl_v3.tex", ///
    replace keep(1.post#c.ln_martyr_per100k_1953) ///
    scalar(Group Controls County_FE Cohort_FE) se r2 ar2 star(* 0.1 ** 0.05 *** 0.01)
...
if "`umodels'" != "" {
    esttab `umodels' using "${proj}/result/table/main_individual_heterogeneity_urbanrural_reghdfejl_v3.rtf", ///
        replace keep(1.post#c.ln_martyr_per100k_1953) ///
        scalar(Group Controls County_FE Cohort_FE) se r2 ar2 star(* 0.1 ** 0.05 *** 0.01)
    esttab `umodels' using "${proj}/result/table/main_individual_heterogeneity_urbanrural_reghdfejl_v3.tex", ///
        replace keep(1.post#c.ln_martyr_per100k_1953) ///
        scalar(Group Controls County_FE Cohort_FE) se r2 ar2 star(* 0.1 ** 0.05 *** 0.01)
}
