*******************************************************
* One-table heterogeneity (reghdfejl)
* Gender (1982/1990/2000), Urban-Rural (1990/2000),
* Relocation subgroup (1982/1990/2000)
*******************************************************
...
capture program drop _prep_wave_once
program define _prep_wave_once
    ...
    keep if inrange(eduy,0,25) & !missing(eduy)
...
capture program drop _build_group_panel
program define _build_group_panel
    syntax , GROUP(string) OUTFILE(string)
...
program define _run_group, rclass
    syntax , PANEL(string) MODEL(string) GROUP(string) WAVE(integer)
...
foreach w in 1982 1990 2000 {
    ...
    quietly _prep_wave_once, wave(`w') hi(`hi')

    tempfile p_male p_female p_reloc0 p_reloc1 p_urban p_rural
    quietly _build_group_panel, group("male") outfile("`p_male'")
    quietly _build_group_panel, group("female") outfile("`p_female'")
    quietly _build_group_panel, group("reloc0") outfile("`p_reloc0'")
    quietly _build_group_panel, group("reloc1") outfile("`p_reloc1'")
    if `w'!=1982 {
        quietly _build_group_panel, group("urban") outfile("`p_urban'")
        quietly _build_group_panel, group("rural") outfile("`p_rural'")
    }

    foreach g in male female reloc0 reloc1 {
        local m = "h`w'_`g'"
        local pfile = "`p_`g''"
        quietly _run_group, panel("`pfile'") model("`m'") group("`g'") wave(`w')
...
gen t_abs = abs(b/se)
sort wave subgroup
export delimited using "${proj}/result/table/main_heterogeneity_one_table_reghdfejl_summary_v2.csv", replace
save "${proj}/result/table/main_heterogeneity_one_table_reghdfejl_summary_v2.dta", replace

esttab `models' using "${proj}/result/table/main_heterogeneity_one_table_reghdfejl_v2.rtf", ///
    replace ///
...
esttab `models' using "${proj}/result/table/main_heterogeneity_one_table_reghdfejl_v2.tex", ///
    replace ///
    keep(1.post#c.ln_martyr_per100k_1953) ///
    scalar(Group Wave County_FE Cohort_FE N_counties) ///
    se r2 ar2 ///
