...
* Coefficients are absolute slopes (vs 0), not normalized in plot.
*******************************************************
...
program define _one_wave, rclass
    ...
    * Base for identification is 1938 (1939 dropped).
    quietly reghdfejl eduy_mean c.ln_martyr_per100k_1953##ib1938.birth_i, ///
        absorb(county_num birth_i) vce(cluster county_num)
    ...
    use "`coef'", clear
    sort birthyr
    export delimited using "${proj}/data/temp/eventstudy_coefficient_drop1939_start1940_`wave'_v1.csv", replace
    save "${proj}/data/temp/eventstudy_coefficient_drop1939_start1940_`wave'_v1.dta", replace
end
...
use "`out'", clear
sort wave
export delimited using "${proj}/data/temp/eventstudy_coefficient_drop1939_start1940_wavecut_summary_v1.csv", replace
save "${proj}/data/temp/eventstudy_coefficient_drop1939_start1940_wavecut_summary_v1.dta", replace
list, clean noobs
