...
capture program drop _run_wave_base1946
program define _run_wave_base1946, rclass
...
quietly _run_wave_base1946, wave(1982) hi(1960)
quietly _run_wave_base1946, wave(1990) hi(1968)
quietly _run_wave_base1946, wave(2000) hi(1978)

esttab m1982 m1990 m2000 using "${proj}/result/table/main_micro_eduy_cutoff1946_rawbaseline_v1.tex", ///
...
tempname memhold
postfile `memhold' str4 wave double b se p N N_counties using "${proj}/result/table/main_micro_eduy_cutoff1946_rawbaseline_v1_tmp.dta", replace
foreach w in 1982 1990 2000 {
    estimates restore m`w'
    post `memhold' ("`w'") (_b[1.post#c.ln_martyr_per100k_1953]) (_se[1.post#c.ln_martyr_per100k_1953]) ///
        (2*ttail(e(df_r), abs(_b[1.post#c.ln_martyr_per100k_1953]/_se[1.post#c.ln_martyr_per100k_1953]))) (e(N)) (e(N_counties))
}
postclose `memhold'
use "${proj}/result/table/main_micro_eduy_cutoff1946_rawbaseline_v1_tmp.dta", clear
export delimited using "${proj}/result/table/main_micro_eduy_cutoff1946_rawbaseline_v1.csv", replace
erase "${proj}/result/table/main_micro_eduy_cutoff1946_rawbaseline_v1_tmp.dta"
exit, clear
