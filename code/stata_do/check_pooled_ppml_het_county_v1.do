...
foreach cl in 0 1 {
    ...
    di "=== `clabel', IHS rate ==="
    ppmlhdfe eduy c.ihs_rate##ib0.post1940 minority i.wave if !drop1939 & clan_any == `cl', ///
    ...
    di "=== `clabel', Dummy ==="
    ppmlhdfe eduy i.d_count_high##ib0.post1940 minority i.wave if !drop1939 & clan_any == `cl', ///
        absorb(county_num birth_i) vce(cluster county_num)
...
/* Output clan table */
tempname fh
file open `fh' using "${outdir}/pooled_ppml_heterogeneity_clan_v1.tex", write replace
file write `fh' "{" _n
...
foreach tp in 0 1 {
    ...
    di "=== `tplabel', IHS rate ==="
    ppmlhdfe eduy c.ihs_rate##ib0.post1940 minority i.wave if !drop1939 & treaty_port_pref == `tp', ///
        absorb(county_num birth_i) vce(cluster county_num)
    ...
    di "=== `tplabel', Dummy ==="
    ppmlhdfe eduy i.d_count_high##ib0.post1940 minority i.wave if !drop1939 & treaty_port_pref == `tp', ///
        absorb(county_num birth_i) vce(cluster county_num)
...
tempname fh
file open `fh' using "${outdir}/pooled_ppml_heterogeneity_treatyport_v1.tex", write replace
file write `fh' "{" _n
file write `fh' "\def\sym#1{\ifmmode^{#1}\else\(\sp{#1}\)\fi}" _n
file write `fh' "\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}l*{4}{c}}" _n
file write `fh' "\toprule" _n
