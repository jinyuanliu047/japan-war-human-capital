...
program define _one_scan, rclass
    ...
    keep if !missing(eduy_mean) & !missing(ln_martyr_raw)
    if `dropyr' > -900 {
    ...
    capture noisily areg eduy_mean c.ln_martyr_raw##ib0.post `timefe', ///
        absorb(county_num) vce(cluster county_num)
    if _rc == 0 {
        capture noisily lincom 1.post#c.ln_martyr_raw
        if _rc == 0 {
    ...
    capture noisily areg eduy_mean ib`base_idx'.rel_idx#c.ln_martyr_raw `timefe', ///
        absorb(county_num) vce(cluster county_num)
    if _rc != 0 {
        return scalar p_pre = .
    ...
    foreach k of local levels {
        local kk = `k' + 50
        if `k' < `base_rel' {
            local preterms `preterms' `kk'.rel_idx#c.ln_martyr_raw
            if inrange(`k', -5, -2) {
                local preterms_near `preterms_near' `kk'.rel_idx#c.ln_martyr_raw
            }
        }
        if `k' >= 0 {
            local postterms `postterms' `kk'.rel_idx#c.ln_martyr_raw
        }
    }
    ...
    if `savecoef' == 1 {
        ...
        foreach k of local levels {
            ...
            if `k' == `base_rel' {
                ...
                local kk = `k' + 50
                capture quietly lincom `kk'.rel_idx#c.ln_martyr_raw
                if _rc == 0 {
        ...
        use "`coef_tmp'", clear
        sort birthyr
        local outstub = "eventstudy_coef_rawcount_`wave'_`spec'"
...
export delimited using "${proj}/data/temp/eventstudy_spec_scan_rawcount_multiwave_v1.csv", replace
save "${proj}/data/temp/eventstudy_spec_scan_rawcount_multiwave_v1.dta", replace
