...
program define _one_scan, rclass
    ...
    capture noisily areg eduy_mean c.ln_martyr_per100k_1953##ib0.post `timefe', ///
        absorb(county_num) vce(cluster county_num)
    if _rc == 0 {
        capture noisily lincom 1.post#c.ln_martyr_per100k_1953
        if _rc == 0 {
    ...
    capture noisily areg eduy_mean ib`base_idx'.rel_idx#c.ln_martyr_per100k_1953 `timefe', ///
        absorb(county_num) vce(cluster county_num)
    if _rc != 0 {
        return scalar p_pre = .
    ...
    foreach k of local levels {
        local kk = `k' + 50
        if `k' < `base_rel' {
            local preterms `preterms' `kk'.rel_idx#c.ln_martyr_per100k_1953
            if inrange(`k', -5, -2) {
                local preterms_near `preterms_near' `kk'.rel_idx#c.ln_martyr_per100k_1953
            }
        }
        if `k' >= 0 {
            local postterms `postterms' `kk'.rel_idx#c.ln_martyr_per100k_1953
        }
    }
    ...
    if `savecoef' == 1 {
        ...
        use "`coef_tmp'", clear
        sort birthyr
        local outstub = "eventstudy_coef_pop1953_unwt_`wave'_`spec'"
...
export delimited using "${proj}/data/temp/eventstudy_spec_scan_pop1953_unwt_multiwave_v1.csv", replace
save "${proj}/data/temp/eventstudy_spec_scan_pop1953_unwt_multiwave_v1.dta", replace
