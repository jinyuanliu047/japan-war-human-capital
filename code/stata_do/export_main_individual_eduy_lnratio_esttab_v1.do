*******************************************************
* Individual-level DID main table (Y = individual eduy)
* Treatment: ln(1 + martyrs per 100k pop1953)
* Specs by wave: no controls / + individual controls
*******************************************************
...
program define _run_wave
    ...
    local model_ctl = ""
    if `wave' == 1982 {
        local model_noc "model1"
        local model_ctl "model2"
    }
    if `wave' == 1990 {
        local model_noc "model3"
        local model_ctl "model4"
    }
    if `wave' == 2000 {
        local model_noc "model5"
        local model_ctl "model6"
    }

    // main_individual_eduy
    quietly areg eduy c.ln_martyr_per100k_1953##ib0.post i.birth_i, absorb(county_num) vce(cluster county_num)
    estimates store `model_noc'
    estadd local Controls "N"
    estadd local County_FE "Y"
    estadd local Cohort_FE "Y"
    estadd local Cluster_SE "County"
    egen __tagc = tag(county_num) if e(sample)
    quietly count if __tagc==1
    estadd scalar N_counties = r(N)
    drop __tagc

    quietly areg eduy c.ln_martyr_per100k_1953##ib0.post i.birth_i $control, absorb(county_num) vce(cluster county_num)
    estimates store `model_ctl'
    estadd local Controls "Y"
    estadd local County_FE "Y"
    estadd local Cohort_FE "Y"
    estadd local Cluster_SE "County"
    egen __tagc2 = tag(county_num) if e(sample)
    quietly count if __tagc2==1
    estadd scalar N_counties = r(N)
    drop __tagc2
end

eststo clear
_run_wave, wave(1982) hi(1960)
_run_wave, wave(1990) hi(1968)
_run_wave, wave(2000) hi(1978)

esttab model1 model2 model3 model4 model5 model6 using "${proj}/result/table/main_individual_eduy_lnratio_esttab_v1.rtf", ///
    replace ///
    keep(1.post#c.ln_martyr_per100k_1953) ///
    scalar(Controls County_FE Cohort_FE Cluster_SE N_counties) ///
    se r2 ar2 ///
    star(* 0.1 ** 0.05 *** 0.01)

esttab model1 model2 model3 model4 model5 model6 using "${proj}/result/table/main_individual_eduy_lnratio_esttab_v1.tex", ///
    replace ///
    keep(1.post#c.ln_martyr_per100k_1953) ///
    scalar(Controls County_FE Cohort_FE Cluster_SE N_counties) ///
    se r2 ar2 ///
