...
*   B) Heterogeneity: memorial_any=0/1
...
keep countyid_curr6 sdy_density ins_famine victims_cr grain_output urbanratio64 ///
    ...
    massacre_event_w ln_massacre_death memorial_cnt_total
replace countyid_curr6 = substr("000000" + countyid_curr6, strlen("000000" + countyid_curr6)-5, 6)
gen memorial_any = memorial_cnt_total > 0 if !missing(memorial_cnt_total)
replace memorial_any = 0 if missing(memorial_any)
...
keep countyid_curr6 memorial_any sdy_density ins_famine victims_cr grain_output urbanratio64 ///
...
foreach x in ln_per100k ihs_per100k raw_count ln_raw share_winsor ihs_share {
    foreach w in 1982 1990 2000 {
        ...
        * B) heterogeneity memorial_any
        quietly reghdfejl eduy_mean c.treat##ib0.post if memorial_any==0, absorb(county_num birth_i) vce(cluster county_num)
        quietly count if memorial_any==0
        local n0 = r(N)
        quietly _post_result, module("hetero_mem") xspec("`x'") wave(`w') label("mem0") ncells(`n0')

        quietly reghdfejl eduy_mean c.treat##ib0.post if memorial_any==1, absorb(county_num birth_i) vce(cluster county_num)
        quietly count if memorial_any==1
        local n1 = r(N)
        quietly _post_result, module("hetero_mem") xspec("`x'") wave(`w') label("mem1") ncells(`n1')

        * C) robustness + reloc#post
...
export delimited using "${proj}/result/table/xspec_fullpack_reghdfejl_summary_v1.csv", replace
save "${proj}/result/table/xspec_fullpack_reghdfejl_summary_v1.dta", replace
