*******************************************************
* Alternative treatment: national anti-war heritage counts
* Y: eduy_mean (county-by-birth cohort unweighted mean)
* X: ln(1 + heritage_site_count)
*******************************************************
...
keep countyid_curr6 ln_heritage_site_count heritage_site_count sdy_density ins_famine victims_cr grain_output urbanratio64 ///
...
capture program drop _run_wave_herit
program define _run_wave_herit, rclass
    ...
    foreach v in ln_heritage_site_count heritage_site_count sdy_density ins_famine victims_cr grain_output urbanratio64 ///
    ...
    reghdfejl eduy_mean c.ln_heritage_site_count##ib0.post, ///
        absorb(county_num prov_cohort c.base_edu_pre#i.birth_i) vce(cluster county_num)
    estimates store herit`wave'_s1
    ...
    return scalar b1 = _b[1.post#c.ln_heritage_site_count]
    return scalar se1 = _se[1.post#c.ln_heritage_site_count]
    return scalar p1 = 2*ttail(e(df_r), abs(_b[1.post#c.ln_heritage_site_count]/_se[1.post#c.ln_heritage_site_count]))
    ...
    reghdfejl eduy_mean c.ln_heritage_site_count##ib0.post ///
        ...
        absorb(county_num prov_cohort c.base_edu_pre#i.birth_i) vce(cluster county_num)
    estimates store herit`wave'_s2
    estadd local FullControls "Y"
    estadd local County_FE "Y"
    estadd local ProvCohort_FE "Y"
    estadd local BaseEduCohort "Y"
    estadd scalar N_counties = r(n_counties)
    return scalar b2 = _b[1.post#c.ln_heritage_site_count]
    return scalar se2 = _se[1.post#c.ln_heritage_site_count]
    return scalar p2 = 2*ttail(e(df_r), abs(_b[1.post#c.ln_heritage_site_count]/_se[1.post#c.ln_heritage_site_count]))
end
...
foreach w in 1982 1990 2000 {
    ...
    quietly _run_wave_herit, wave(`w') hi(`hi')
...
gen t_abs = abs(b/se)
sort wave spec
export delimited using "${proj}/result/table/alt_treatment_heritage_count_summary_v1.csv", replace
save "${proj}/result/table/alt_treatment_heritage_count_summary_v1.dta", replace

esttab herit1982_s1 herit1982_s2 herit1990_s1 herit1990_s2 herit2000_s1 herit2000_s2 ///
    using "${proj}/result/table/alt_treatment_heritage_count_v1.rtf", ///
    replace ///
    keep(1.post#c.ln_heritage_site_count) ///
    scalar(FullControls County_FE ProvCohort_FE BaseEduCohort N_counties) ///
    se r2 ar2 ///
    star(* 0.1 ** 0.05 *** 0.01)

esttab herit1982_s1 herit1982_s2 herit1990_s1 herit1990_s2 herit2000_s1 herit2000_s2 ///
    using "${proj}/result/table/alt_treatment_heritage_count_v1.tex", ///
    replace ///
    keep(1.post#c.ln_heritage_site_count) ///
    scalar(FullControls County_FE ProvCohort_FE BaseEduCohort N_counties) ///
    se r2 ar2 ///
