*******************************************************
* Alternative treatment: national anti-war heritage any
...
capture program drop _run_wave_heritany
program define _run_wave_heritany, rclass
    ...
    return scalar b1 = _b[1.post#c.heritage_any]
    return scalar se1 = _se[1.post#c.heritage_any]
    return scalar p1 = 2*ttail(e(df_r), abs(_b[1.post#c.heritage_any]/_se[1.post#c.heritage_any]))
    ...
    reghdfejl eduy_mean c.heritage_any##ib0.post ///
        ...
        absorb(county_num prov_cohort c.base_edu_pre#i.birth_i) vce(cluster county_num)
    estimates store heritany`wave'_s2
    estadd local FullControls "Y"
    estadd local County_FE "Y"
    estadd local ProvCohort_FE "Y"
    estadd local BaseEduCohort "Y"
    estadd scalar N_counties = r(n_counties)
    return scalar b2 = _b[1.post#c.heritage_any]
    return scalar se2 = _se[1.post#c.heritage_any]
    return scalar p2 = 2*ttail(e(df_r), abs(_b[1.post#c.heritage_any]/_se[1.post#c.heritage_any]))
end
...
foreach w in 1982 1990 2000 {
    ...
    quietly _run_wave_heritany, wave(`w') hi(`hi')
...
gen t_abs = abs(b/se)
sort wave spec
export delimited using "${proj}/result/table/alt_treatment_heritage_any_summary_v1.csv", replace
save "${proj}/result/table/alt_treatment_heritage_any_summary_v1.dta", replace

esttab heritany1982_s1 heritany1982_s2 heritany1990_s1 heritany1990_s2 heritany2000_s1 heritany2000_s2 ///
    using "${proj}/result/table/alt_treatment_heritage_any_v1.rtf", ///
    replace ///
    keep(1.post#c.heritage_any) ///
    scalar(FullControls County_FE ProvCohort_FE BaseEduCohort N_counties) ///
    se r2 ar2 ///
    star(* 0.1 ** 0.05 *** 0.01)

esttab heritany1982_s1 heritany1982_s2 heritany1990_s1 heritany1990_s2 heritany2000_s1 heritany2000_s2 ///
    using "${proj}/result/table/alt_treatment_heritage_any_v1.tex", ///
    replace ///
    keep(1.post#c.heritage_any) ///
    scalar(FullControls County_FE ProvCohort_FE BaseEduCohort N_counties) ///
    se r2 ar2 ///
