/*--------------------------------------------------------------
  Individual-level regressions: ln vs IHS treatment, OLS vs PPML
...
*--------------------------------------------------------------
* treatment: load both ln and raw, construct IHS
*--------------------------------------------------------------
use "${proj}/data/temp/martyr_pop1953_treatment_county_v2.dta", clear
keep countyid_curr6 ln_martyr_per100k_1953
* Try to keep raw variable; if not present, back it out from ln
capture confirm variable martyr_per100k_1953
if _rc == 0 {
    * Variable exists in dataset – reload with it
    use "${proj}/data/temp/martyr_pop1953_treatment_county_v2.dta", clear
    capture keep countyid_curr6 ln_martyr_per100k_1953 martyr_per100k_1953
}
else {
    * Back out raw from ln: ln_martyr = ln(raw + 1) => raw = exp(ln) - 1
    gen martyr_per100k_1953 = exp(ln_martyr_per100k_1953) - 1
}
...
*--------------------------------------------------------------
* Initialise result locals
*--------------------------------------------------------------
local waves "1982 1990 2000"
foreach w of local waves {
    foreach m in olsln olsihs ppmlln ppmlihs {
        local b_`w'_`m' .
        local se_`w'_`m' .
        local p_`w'_`m' .
        local n_`w'_`m' .
...
* (a) OLS with ln treatment
di "=== 1982: OLS ln ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) vce(cluster county_num)
local b_1982_olsln = _b[1.post#c.ln_martyr_per100k_1953]
local se_1982_olsln = _se[1.post#c.ln_martyr_per100k_1953]
local p_1982_olsln = 2*ttail(e(df_r), abs(`b_1982_olsln'/`se_1982_olsln'))
local n_1982_olsln = e(N)

* (b) OLS with IHS treatment
di "=== 1982: OLS IHS ==="
quietly reghdfejl eduy c.ihs_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) vce(cluster county_num)
local b_1982_olsihs = _b[1.post#c.ihs_martyr_per100k_1953]
local se_1982_olsihs = _se[1.post#c.ihs_martyr_per100k_1953]
local p_1982_olsihs = 2*ttail(e(df_r), abs(`b_1982_olsihs'/`se_1982_olsihs'))
local n_1982_olsihs = e(N)

* (c) PPML with ln treatment
di "=== 1982: PPML ln ==="
quietly ppmlhdfe eduy c.ln_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) cluster(county_num)
local b_1982_ppmlln = _b[1.post#c.ln_martyr_per100k_1953]
local se_1982_ppmlln = _se[1.post#c.ln_martyr_per100k_1953]
local p_1982_ppmlln = 2*normal(-abs(`b_1982_ppmlln'/`se_1982_ppmlln'))
local n_1982_ppmlln = e(N)

* (d) PPML with IHS treatment
di "=== 1982: PPML IHS ==="
quietly ppmlhdfe eduy c.ihs_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) cluster(county_num)
local b_1982_ppmlihs = _b[1.post#c.ihs_martyr_per100k_1953]
local se_1982_ppmlihs = _se[1.post#c.ihs_martyr_per100k_1953]
local p_1982_ppmlihs = 2*normal(-abs(`b_1982_ppmlihs'/`se_1982_ppmlihs'))
local n_1982_ppmlihs = e(N)
...
di "=== 1990: OLS ln ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) vce(cluster county_num)
local b_1990_olsln = _b[1.post#c.ln_martyr_per100k_1953]
local se_1990_olsln = _se[1.post#c.ln_martyr_per100k_1953]
local p_1990_olsln = 2*ttail(e(df_r), abs(`b_1990_olsln'/`se_1990_olsln'))
local n_1990_olsln = e(N)

* (b) OLS with IHS treatment
di "=== 1990: OLS IHS ==="
quietly reghdfejl eduy c.ihs_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) vce(cluster county_num)
local b_1990_olsihs = _b[1.post#c.ihs_martyr_per100k_1953]
local se_1990_olsihs = _se[1.post#c.ihs_martyr_per100k_1953]
local p_1990_olsihs = 2*ttail(e(df_r), abs(`b_1990_olsihs'/`se_1990_olsihs'))
local n_1990_olsihs = e(N)

* (c) PPML with ln treatment
di "=== 1990: PPML ln ==="
quietly ppmlhdfe eduy c.ln_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) cluster(county_num)
local b_1990_ppmlln = _b[1.post#c.ln_martyr_per100k_1953]
local se_1990_ppmlln = _se[1.post#c.ln_martyr_per100k_1953]
local p_1990_ppmlln = 2*normal(-abs(`b_1990_ppmlln'/`se_1990_ppmlln'))
local n_1990_ppmlln = e(N)

* (d) PPML with IHS treatment
di "=== 1990: PPML IHS ==="
quietly ppmlhdfe eduy c.ihs_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) cluster(county_num)
local b_1990_ppmlihs = _b[1.post#c.ihs_martyr_per100k_1953]
local se_1990_ppmlihs = _se[1.post#c.ihs_martyr_per100k_1953]
local p_1990_ppmlihs = 2*normal(-abs(`b_1990_ppmlihs'/`se_1990_ppmlihs'))
local n_1990_ppmlihs = e(N)
...
di "=== 2000: OLS ln ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) vce(cluster county_num)
local b_2000_olsln = _b[1.post#c.ln_martyr_per100k_1953]
local se_2000_olsln = _se[1.post#c.ln_martyr_per100k_1953]
local p_2000_olsln = 2*ttail(e(df_r), abs(`b_2000_olsln'/`se_2000_olsln'))
local n_2000_olsln = e(N)

* (b) OLS with IHS treatment
di "=== 2000: OLS IHS ==="
quietly reghdfejl eduy c.ihs_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) vce(cluster county_num)
local b_2000_olsihs = _b[1.post#c.ihs_martyr_per100k_1953]
local se_2000_olsihs = _se[1.post#c.ihs_martyr_per100k_1953]
local p_2000_olsihs = 2*ttail(e(df_r), abs(`b_2000_olsihs'/`se_2000_olsihs'))
local n_2000_olsihs = e(N)

* (c) PPML with ln treatment
di "=== 2000: PPML ln ==="
quietly ppmlhdfe eduy c.ln_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) cluster(county_num)
local b_2000_ppmlln = _b[1.post#c.ln_martyr_per100k_1953]
local se_2000_ppmlln = _se[1.post#c.ln_martyr_per100k_1953]
local p_2000_ppmlln = 2*normal(-abs(`b_2000_ppmlln'/`se_2000_ppmlln'))
local n_2000_ppmlln = e(N)

* (d) PPML with IHS treatment
di "=== 2000: PPML IHS ==="
quietly ppmlhdfe eduy c.ihs_martyr_per100k_1953##ib0.post $control, absorb(county_num birth_i) cluster(county_num)
local b_2000_ppmlihs = _b[1.post#c.ihs_martyr_per100k_1953]
local se_2000_ppmlihs = _se[1.post#c.ihs_martyr_per100k_1953]
local p_2000_ppmlihs = 2*normal(-abs(`b_2000_ppmlihs'/`se_2000_ppmlihs'))
local n_2000_ppmlihs = e(N)

di "=== 2000 done ==="

*--------------------------------------------------------------
* Export .tex table (two panels: OLS and PPML)
*--------------------------------------------------------------
...
local texfile "${proj}/paper/assets/tables/main_individual_ihs_ppml_reghdfejl_v1.tex"
...
file write texout "            &\multicolumn{1}{c}{ln}&\multicolumn{1}{c}{IHS}&\multicolumn{1}{c}{ln}&\multicolumn{1}{c}{IHS}&\multicolumn{1}{c}{ln}&\multicolumn{1}{c}{IHS}\\" _n
file write texout "\midrule" _n

* ---- Panel A: OLS (reghdfejl) ----
file write texout "\multicolumn{7}{l}{\textit{Panel A: OLS (reghdfejl)}}\\" _n
file write texout "[0.3em]" _n

* Stars for OLS
foreach w in 1982 1990 2000 {
    foreach m in olsln olsihs {
        _stars `p_`w'_`m''
        local s_`w'_`m' = r(star)
    }
}
...
foreach w in 1982 1990 2000 {
    foreach m in olsln olsihs {
        local bv : di %9.3f `b_`w'_`m''
        file write texout "&" %14s "`bv'\sym{`s_`w'_`m''}"
    }
}
...
foreach w in 1982 1990 2000 {
    foreach m in olsln olsihs {
        local sev : di %9.3f `se_`w'_`m''
        file write texout "&" %14s "(`sev')"
    }
}
file write texout "\\" _n
file write texout "[0.5em]" _n

* ---- Panel B: PPML ----
file write texout "\multicolumn{7}{l}{\textit{Panel B: PPML (ppmlhdfe)}}\\" _n
file write texout "[0.3em]" _n

* Stars for PPML
foreach w in 1982 1990 2000 {
    foreach m in ppmlln ppmlihs {
        _stars `p_`w'_`m''
        local s_`w'_`m' = r(star)
    }
}

* Coefficient row – PPML
file write texout "Post \$\times\$ War exp."
foreach w in 1982 1990 2000 {
    foreach m in ppmlln ppmlihs {
        local bv : di %9.3f `b_`w'_`m''
...
* Observations (use OLS N, which should equal PPML N)
file write texout "\(N\)       "
foreach w in 1982 1990 2000 {
    foreach m in olsln olsihs {
        local nv : di %12.0fc `n_`w'_`m''
