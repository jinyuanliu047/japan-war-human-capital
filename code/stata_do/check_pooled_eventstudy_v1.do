/*  ================================================================
    Pooled Event Study: cohort-by-cohort interaction coefficients
    Uses mainvars files (faster, no sex needed)
    ================================================================ */
...
*==============================================================
*  Event study: cohort-by-cohort interaction with IHS(rate)
*  Base year = 1939 (dropped)
*==============================================================
forvalues yr = 1920/1956 {
    gen d`yr' = (birth_i == `yr')
    gen d`yr'_X = d`yr' * ihs_rate
}
drop d1939 d1939_X

di "=== Running event study regression ==="
reghdfejl eduy d*_X minority i.wave, absorb(county_num birth_i) vce(cluster county_num)
...
twoway (rarea ci_lo ci_hi year, color(navy%20) lwidth(none)) ///
       (connected beta year, mcolor(navy) lcolor(navy) msize(small) lwidth(medthin)) ///
       (scatteri 0 1939, mcolor(red) msize(medlarge) msymbol(diamond)), ///
    xline(1939.5, lcolor(red) lpattern(dash)) ///
    xline(1945.5, lcolor(orange) lpattern(dash)) ///
    yline(0, lcolor(gs8) lpattern(solid)) ///
    xlabel(1920(5)1955, labsize(small)) ///
    ylabel(, labsize(small) format(%5.3f)) ///
    xtitle("Birth cohort", size(medsmall)) ///
    ytitle("Coefficient on IHS(martyrs per 100k) x cohort", size(medsmall)) ///
    title("") ///
    note("Base year: 1939 (red diamond). Shaded area: 95% CI, SE clustered at county level." ///
         "County, birth-year, and wave FE absorbed. Red dashed: 1940 cutoff; orange dashed: 1946.", size(vsmall)) ///
    legend(off) ///
