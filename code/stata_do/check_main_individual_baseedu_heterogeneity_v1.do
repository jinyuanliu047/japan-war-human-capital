/*--------------------------------------------------------------
  Individual-level heterogeneity: base education tercile (Low/Mid/High)
...
*--------------------------------------------------------------
* base education tercile data (wave-specific)
*--------------------------------------------------------------
...
merge m:1 countyid_curr6 using "$base1982_tmp", keep(master match) nogen keepusing(baseedu_tercile)
gen birth_i = floor(birthyr)
gen post = birth_i >= 1940
egen county_num = group(countyid_curr6)

di "=== 1982: baseedu_tercile == 1 (Low) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==1, absorb(county_num birth_i) vce(cluster county_num)
local b_1982_B1 = _b[1.post#c.ln_martyr_per100k_1953]
local se_1982_B1 = _se[1.post#c.ln_martyr_per100k_1953]
local p_1982_B1 = 2*ttail(e(df_r), abs(`b_1982_B1'/`se_1982_B1'))
local n_1982_B1 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_1982_B1 = r(N)
drop tagc

di "=== 1982: baseedu_tercile == 2 (Mid) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==2, absorb(county_num birth_i) vce(cluster county_num)
local b_1982_B2 = _b[1.post#c.ln_martyr_per100k_1953]
local se_1982_B2 = _se[1.post#c.ln_martyr_per100k_1953]
local p_1982_B2 = 2*ttail(e(df_r), abs(`b_1982_B2'/`se_1982_B2'))
local n_1982_B2 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_1982_B2 = r(N)
drop tagc

di "=== 1982: baseedu_tercile == 3 (High) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==3, absorb(county_num birth_i) vce(cluster county_num)
local b_1982_B3 = _b[1.post#c.ln_martyr_per100k_1953]
local se_1982_B3 = _se[1.post#c.ln_martyr_per100k_1953]
local p_1982_B3 = 2*ttail(e(df_r), abs(`b_1982_B3'/`se_1982_B3'))
local n_1982_B3 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_1982_B3 = r(N)
drop tagc
di "=== 1982 done: B1 b=`b_1982_B1' n=`n_1982_B1'; B2 b=`b_1982_B2' n=`n_1982_B2'; B3 b=`b_1982_B3' n=`n_1982_B3' ==="
...
keep if inrange(eduy, 0, 25)
merge m:1 countyid_curr6 using "$treat_tmp", keep(master match) nogen
merge m:1 countyid_curr6 using "$base1990_tmp", keep(master match) nogen keepusing(baseedu_tercile)
gen birth_i = floor(birthyr)
gen post = birth_i >= 1940
egen county_num = group(countyid_curr6)

di "=== 1990: baseedu_tercile == 1 (Low) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==1, absorb(county_num birth_i) vce(cluster county_num)
local b_1990_B1 = _b[1.post#c.ln_martyr_per100k_1953]
local se_1990_B1 = _se[1.post#c.ln_martyr_per100k_1953]
local p_1990_B1 = 2*ttail(e(df_r), abs(`b_1990_B1'/`se_1990_B1'))
local n_1990_B1 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_1990_B1 = r(N)
drop tagc

di "=== 1990: baseedu_tercile == 2 (Mid) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==2, absorb(county_num birth_i) vce(cluster county_num)
local b_1990_B2 = _b[1.post#c.ln_martyr_per100k_1953]
local se_1990_B2 = _se[1.post#c.ln_martyr_per100k_1953]
local p_1990_B2 = 2*ttail(e(df_r), abs(`b_1990_B2'/`se_1990_B2'))
local n_1990_B2 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_1990_B2 = r(N)
drop tagc

di "=== 1990: baseedu_tercile == 3 (High) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==3, absorb(county_num birth_i) vce(cluster county_num)
local b_1990_B3 = _b[1.post#c.ln_martyr_per100k_1953]
local se_1990_B3 = _se[1.post#c.ln_martyr_per100k_1953]
local p_1990_B3 = 2*ttail(e(df_r), abs(`b_1990_B3'/`se_1990_B3'))
local n_1990_B3 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_1990_B3 = r(N)
drop tagc
di "=== 1990 done: B1 b=`b_1990_B1' n=`n_1990_B1'; B2 b=`b_1990_B2' n=`n_1990_B2'; B3 b=`b_1990_B3' n=`n_1990_B3' ==="
...
keep if inrange(eduy, 0, 25)
merge m:1 countyid_curr6 using "$treat_tmp", keep(master match) nogen
merge m:1 countyid_curr6 using "$base2000_tmp", keep(master match) nogen keepusing(baseedu_tercile)
gen birth_i = floor(birthyr)
gen post = birth_i >= 1940
egen county_num = group(countyid_curr6)

di "=== 2000: baseedu_tercile == 1 (Low) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==1, absorb(county_num birth_i) vce(cluster county_num)
local b_2000_B1 = _b[1.post#c.ln_martyr_per100k_1953]
local se_2000_B1 = _se[1.post#c.ln_martyr_per100k_1953]
local p_2000_B1 = 2*ttail(e(df_r), abs(`b_2000_B1'/`se_2000_B1'))
local n_2000_B1 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_2000_B1 = r(N)
drop tagc

di "=== 2000: baseedu_tercile == 2 (Mid) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==2, absorb(county_num birth_i) vce(cluster county_num)
local b_2000_B2 = _b[1.post#c.ln_martyr_per100k_1953]
local se_2000_B2 = _se[1.post#c.ln_martyr_per100k_1953]
local p_2000_B2 = 2*ttail(e(df_r), abs(`b_2000_B2'/`se_2000_B2'))
local n_2000_B2 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_2000_B2 = r(N)
drop tagc

di "=== 2000: baseedu_tercile == 3 (High) ==="
quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post $control if baseedu_tercile==3, absorb(county_num birth_i) vce(cluster county_num)
local b_2000_B3 = _b[1.post#c.ln_martyr_per100k_1953]
local se_2000_B3 = _se[1.post#c.ln_martyr_per100k_1953]
local p_2000_B3 = 2*ttail(e(df_r), abs(`b_2000_B3'/`se_2000_B3'))
local n_2000_B3 = e(N)
egen tagc = tag(county_num) if e(sample)
count if tagc==1
local nc_2000_B3 = r(N)
drop tagc
di "=== 2000 done: B1 b=`b_2000_B1' n=`n_2000_B1'; B2 b=`b_2000_B2' n=`n_2000_B2'; B3 b=`b_2000_B3' n=`n_2000_B3' ==="
...
local texfile "${proj}/paper/assets/tables/main_individual_baseedu_heterogeneity_reghdfejl_v1.tex"

capture file close texout
file open texout using "`texfile'", write replace

file write texout "{\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi}" _n
file write texout "\begin{tabular}{l*{9}{c}}" _n
file write texout "\toprule" _n
file write texout "            &\multicolumn{3}{c}{1982}&\multicolumn{3}{c}{1990}&\multicolumn{3}{c}{2000}\\" _n
file write texout "\cmidrule(lr){2-4}\cmidrule(lr){5-7}\cmidrule(lr){8-10}" _n
file write texout "            &\multicolumn{1}{c}{Low-edu}&\multicolumn{1}{c}{Mid-edu}&\multicolumn{1}{c}{High-edu}&\multicolumn{1}{c}{Low-edu}&\multicolumn{1}{c}{Mid-edu}&\multicolumn{1}{c}{High-edu}&\multicolumn{1}{c}{Low-edu}&\multicolumn{1}{c}{Mid-edu}&\multicolumn{1}{c}{High-edu}\\" _n
