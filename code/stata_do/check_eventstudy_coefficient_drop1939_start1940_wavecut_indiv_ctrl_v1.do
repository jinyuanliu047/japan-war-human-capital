...
* Individual-level eduy (no weights), with controls
* Drop cohort 1939
...
*******************************************************
* helper: one wave on individual data with controls
*******************************************************
capture program drop _one_wave_indiv_ctrl
program define _one_wave_indiv_ctrl, rclass
    ...
    quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib0.post minority, ///
    ...
    quietly reghdfejl eduy c.ln_martyr_per100k_1953##ib1938.birth_i minority, ///
        absorb(county_num birth_i) vce(cluster county_num)
    ...
    use "`coef'", clear
    sort birthyr
    export delimited using "${proj}/data/temp/eventstudy_coefficient_drop1939_start1940_`wave'_indiv_ctrl_v1.csv", replace
    save "${proj}/data/temp/eventstudy_coefficient_drop1939_start1940_`wave'_indiv_ctrl_v1.dta", replace
end
...
foreach w in 1982 1990 2000 {
    ...
    quietly _one_wave_indiv_ctrl, wave(`w') hi(`hi')
...
use `summary', clear
sort wave
export delimited using "${proj}/data/temp/eventstudy_coefficient_drop1939_start1940_wavecut_summary_indiv_ctrl_v1.csv", replace
save "${proj}/data/temp/eventstudy_coefficient_drop1939_start1940_wavecut_summary_indiv_ctrl_v1.dta", replace
list, clean noobs
