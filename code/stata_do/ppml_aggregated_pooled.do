/*  ================================================================
    ULTRA-FAST POOLED PPML: AGGREGATED SAMPLE (FINAL STABLE)
    ================================================================ */
clear all
set more off
...
keep countyid_curr6 martyr_per100k_1953 ln_martyr_per100k_1953
...
rename countyid_curr6 countyid
tostring countyid, replace force
replace countyid = substr("000000" + countyid, strlen("000000" + countyid)-5, 6)
tempfile treat
save `treat'
...
capture drop _merge
capture rename countyid_curr6 countyid
tostring countyid, replace force
replace countyid = substr("000000" + countyid, strlen("000000" + countyid)-5, 6)
gen wave = 1
tempfile w1
...
use "${proj}/data/temp/did_2000_county_birthyr_pop1953_unwt_v2_rebuilt.dta", clear
capture drop _merge
capture rename countyid_curr6 countyid
tostring countyid, replace force
replace countyid = substr("000000" + countyid, strlen("000000" + countyid)-5, 6)
...
* 3. MERGE AND CLEAN
merge m:1 countyid using `treat', keep(match) nogen
capture rename birthyr birth_i
capture rename eduy_mean eduy
encode countyid, gen(county_num)
...
* 4. RUN ALL PPML REGRESSIONS
foreach cutoff in 1940 1946 {
    local dropyr = `cutoff' - 1
    gen post = (birth_i >= `cutoff')
    gen drop_yr = (birth_i == `dropyr')
    ppmlhdfe eduy c.ihs_rate##ib0.post i.wave if !drop_yr, absorb(county_num birth_i) vce(cluster county_num)
    ...
    ppmlhdfe eduy i.d_count_high##ib0.post i.wave if !drop_yr, absorb(county_num birth_i) vce(cluster county_num)
