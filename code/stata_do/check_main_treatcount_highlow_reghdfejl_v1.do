*******************************************************
* Heterogeneity by raw martyr count high/low split
...
program define _prep_wave
    ...
    xtile __bin = __raw if __tag==1, nq(2)
    by county_num: egen treatcount_bin = max(__bin)
    label define treatbin 1 "low" 2 "high"
    label values treatcount_bin treatbin
    drop __raw __tag __bin
end
...
program define _run_one, rclass
    ...
    use "`panel'", clear
    quietly count if treatcount_bin==`groupval'
    return scalar n_cells = r(N)

    quietly reghdfejl eduy_mean c.ln_martyr_per100k_1953##ib0.post if treatcount_bin==`groupval', ///
...
foreach w in 1982 1990 2000 {
    ...
    foreach g in 1 2 {
        local glab = cond(`g'==1, "RCL", "RCH")
        local m = "rchl`w'_`g'"
        quietly _run_one, panel("`wpanel'") groupval(`g') model("`m'") glabel("`glab'")
        local models "`models' `m'"
        use `summary', clear
        local n = _N + 1
        set obs `n'
        replace wave = "`w'" in `n'
        replace dim = "rawcount_highlow" in `n'
        replace grp = "`glab'" in `n'
        replace b = r(b) in `n'
...
gen t_abs = abs(b/se)
sort wave grp
export delimited using "${proj}/result/table/main_treatcount_highlow_reghdfejl_summary_v1.csv", replace
save "${proj}/result/table/main_treatcount_highlow_reghdfejl_summary_v1.dta", replace

esttab `models' using "${proj}/result/table/main_treatcount_highlow_reghdfejl_v1.rtf", ///
    replace ///
...
esttab `models' using "${proj}/result/table/main_treatcount_highlow_reghdfejl_v1.tex", ///
    replace ///
    keep(1.post#c.ln_martyr_per100k_1953) ///
    scalar(Group County_FE Cohort_FE) ///
    se r2 ar2 ///
