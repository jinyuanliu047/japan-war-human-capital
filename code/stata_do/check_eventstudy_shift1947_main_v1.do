*******************************************************
* Shifted-base event-study check (main spec)
* Shift all by +1 year:
*   - base cohort: 1946
*   - treatment starts: 1947
*   - sample window: 1932-1961
*******************************************************
...
program define _one_shift, rclass
    ...
    gen rel = birthyr - 1947
    gen rel_bin = rel
    replace rel_bin = -8 if rel <= -8
    replace rel_bin = 8 if rel >= 8
    gen rel_idx = rel_bin + 50

    * base t=-1 -> cohort 1946 -> rel=-1 -> idx 49
    ...
    foreach k of local levels {
        local yy = 1947 + `k'
        if `k' == -1 {
            post `ph' ("`wave'") (`yy') (`k') (0) (0) (0) (0) (1947) (1946) (1932) (1961)
        }
        else {
            local kk = `k' + 50
    ...
    use "`coef'", clear
    sort birthyr
    export delimited using "${proj}/data/temp/eventstudy_coef_shift1947_main_`wave'_v1.csv", replace
    save "${proj}/data/temp/eventstudy_coef_shift1947_main_`wave'_v1.dta", replace
end
...
foreach w in 1982 1990 2000 {
    ...
    quietly _one_shift, wave("`w'") datafile("`d'")
    local b_did = r(b_did)
    ...
    post `posth' ("`w'") (1947) (1946) (1932) (1961) ///
...
use "`out'", clear
sort wave
export delimited using "${proj}/data/temp/eventstudy_shift1947_main_summary_v1.csv", replace
save "${proj}/data/temp/eventstudy_shift1947_main_summary_v1.dta", replace
list, clean noobs
