...
* 3) National anti-war heritage site: heritage_any (0/1)
...
keep countyid_curr6 ln_massacre_death
...
use "${proj}/data/temp/heritage_treat.dta", clear
keep countyid treat
tostring countyid, replace format(%06.0f) force
rename countyid countyid_curr6
gen heritage_any = treat > 0 if !missing(treat)
replace heritage_any = 0 if missing(heritage_any)
keep countyid_curr6 heritage_any
duplicates drop countyid_curr6, force
tempfile heritage
save `heritage'

use `ctrl', clear
merge 1:1 countyid_curr6 using `heritage', keep(master match) nogen
replace heritage_any = 0 if missing(heritage_any)
...
program define _prep_wave
    ...
    merge m:1 countyid_curr6 using "$het_tmp", keep(master match) nogen
    replace heritage_any = 0 if missing(heritage_any)
    replace ln_massacre_death = 0 if missing(ln_massacre_death)
...
gen t_abs = abs(b/se)
sort wave dim grp
export delimited using "${proj}/result/table/main_heterogeneity_pack_heritage_reghdfejl_summary_v1.csv", replace
save "${proj}/result/table/main_heterogeneity_pack_heritage_reghdfejl_summary_v1.dta", replace

esttab `models' using "${proj}/result/table/main_heterogeneity_pack_heritage_reghdfejl_v1.rtf", ///
    replace ///
...
esttab `models' using "${proj}/result/table/main_heterogeneity_pack_heritage_reghdfejl_v1.tex", ///
    replace ///
    keep(1.post#c.ln_martyr_per100k_1953) ///
    scalar(Dim Group County_FE Cohort_FE) ///
    se r2 ar2 ///
