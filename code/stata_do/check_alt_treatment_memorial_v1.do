*******************************************************
* Alternative treatment: memorial facility intensity
* Y: eduy_mean (unweighted county-by-birth cohort mean)
* Treatment: ln(1 + memorial facility count)
*******************************************************
...
keep countyid_curr6 ln_memorial_cnt sdy_density ins_famine victims_cr grain_output urbanratio64 ///
...
capture program drop _run_wave_alt
program define _run_wave_alt, rclass
    ...
    foreach v in ln_memorial_cnt sdy_density ins_famine victims_cr grain_output urbanratio64 ///
    ...
    // alt_treatment_memorial
    reghdfejl eduy_mean c.ln_memorial_cnt##ib0.post, ///
        absorb(county_num prov_cohort c.base_edu_pre#i.birth_i) vce(cluster county_num)
    estimates store alt`wave'_s1
    ...
    return scalar b1 = _b[1.post#c.ln_memorial_cnt]
    return scalar se1 = _se[1.post#c.ln_memorial_cnt]
    return scalar p1 = 2*ttail(e(df_r), abs(_b[1.post#c.ln_memorial_cnt]/_se[1.post#c.ln_memorial_cnt]))
    ...
    reghdfejl eduy_mean c.ln_memorial_cnt##ib0.post ///
        ...
        absorb(county_num prov_cohort c.base_edu_pre#i.birth_i) vce(cluster county_num)
    estimates store alt`wave'_s2
    estadd local FullControls "Y"
    estadd local County_FE "Y"
    estadd local ProvCohort_FE "Y"
    estadd local BaseEduCohort "Y"
    estadd scalar N_counties = `n_counties'
    return scalar b2 = _b[1.post#c.ln_memorial_cnt]
    return scalar se2 = _se[1.post#c.ln_memorial_cnt]
    return scalar p2 = 2*ttail(e(df_r), abs(_b[1.post#c.ln_memorial_cnt]/_se[1.post#c.ln_memorial_cnt]))
end
...
foreach w in 1982 1990 2000 {
    ...
    quietly _run_wave_alt, wave(`w') hi(`hi')
...
gen t_abs = abs(b/se)
sort wave spec
export delimited using "${proj}/result/table/alt_treatment_memorial_summary_v1.csv", replace
save "${proj}/result/table/alt_treatment_memorial_summary_v1.dta", replace

esttab alt1982_s1 alt1982_s2 alt1990_s1 alt1990_s2 alt2000_s1 alt2000_s2 ///
    using "${proj}/result/table/alt_treatment_memorial_v1.rtf", ///
    replace ///
    keep(1.post#c.ln_memorial_cnt) ///
    scalar(FullControls County_FE ProvCohort_FE BaseEduCohort N_counties) ///
    se r2 ar2 ///
    star(* 0.1 ** 0.05 *** 0.01)

esttab alt1982_s1 alt1982_s2 alt1990_s1 alt1990_s2 alt2000_s1 alt2000_s2 ///
    using "${proj}/result/table/alt_treatment_memorial_v1.tex", ///
    replace ///
    keep(1.post#c.ln_memorial_cnt) ///
    scalar(FullControls County_FE ProvCohort_FE BaseEduCohort N_counties) ///
    se r2 ar2 ///
