*******************************************************
* Event-study check: drop 1945, treated cohorts >=1946
* Waves: 1982 / 1990 / 2000
* Sample window: 1931-1960, excluding 1945
* Base cohort in ES: 1944 (last kept pre cohort)
*******************************************************
...
program define _one_run, rclass
    ...
    * base cohort = 1944 => rel = -2 => idx = 48
    quietly areg eduy_mean ib48.rel_idx#c.ln_martyr_per100k_1953 i.birthyr, ///
        absorb(county_num) vce(cluster county_num)
    ...
    foreach k of local levels {
        local kk = `k' + 50
        if `k' <= -3 local preterms `preterms' `kk'.rel_idx#c.ln_martyr_per100k_1953
    ...
    foreach k of local levels {
        local yy = 1946 + `k'
        if `k' == -2 {
            post `ph' ("`wave'") (`yy') (`k') (0) (0) (0) (0) (1946) (1944) (1931) (1960)
        }
        else {
            local kk = `k' + 50
    ...
    use "`coef'", clear
    sort birthyr
    export delimited using "${proj}/data/temp/eventstudy_coef_drop1945_start1946_main_`wave'_v1.csv", replace
    save "${proj}/data/temp/eventstudy_coef_drop1945_start1946_main_`wave'_v1.dta", replace
end
...
foreach w in 1982 1990 2000 {
    ...
    post `posth' ("`w'") (1946) (1944) (1931) (1960) ///
...
use "`out'", clear
sort wave
export delimited using "${proj}/data/temp/eventstudy_drop1945_start1946_main_summary_v1.csv", replace
save "${proj}/data/temp/eventstudy_drop1945_start1946_main_summary_v1.dta", replace
list, clean noobs
